...
Vi kan altså sammenligne **lovforslag** ... men ikke partier eller politikere, som vi har preprocesseret data indtil nu. Det er en **designbegrænsning**.
...
- Det, vi vil måle er forekomster af tema **per 1.000 ord**
...
Gense noter om `.melt()` fra lektion 3. Vi går fra én kolonne per tema til én række per tema og år. Det er formatet, plotnine vil have.
...
Samme idé som vores egen ordbog, men med en pipeline under sig.
...
::: {.aside}
`sentencizer` er nødvendig. `asent` regner en score per sætning og lægger dem sammen, så den skal vide, hvor sætningerne slutter. Uden den får I `ValueError: Sentence boundaries unset`.
:::
...
Fx er `god` i sig selv positiv. Den bliver negativ, fordi `ikke` står foran. Sådanne fund kan slås op i ordbogsmodeller. 
...
Tag $N$ debatter og læs dem, og vurder for hver enkelt, om I er enige med modellen.
...
Bemærk den nye øvre grænse, hvor ord, der står i over halvdelen af debatterne, fjernes. 
...
`n_clusters=6` er **vores** valg, i en øvelseskontekst vi ved, der er seks områder. **I en "rigtig" analyse ved man det ikke.**
